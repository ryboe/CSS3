...
class CSS3Completions(sublime_plugin.EventListener):
    def on_query_completions(self, view, prefix, locations):
        ...
        if view.match_selector(
            start, "meta.declaration-list.css -meta.selector.css -meta.property-value."
        ):
        ...
        # @IMPORT
        if view.match_selector(
            start, "meta.at-rule.import.css -meta.media-feature.css"
        ):
            return (
                types.media_types + [types.string, types.url],
        ...
        if view.match_selector(
            start, "meta.at-rule.font-face.block.css -meta.descriptor.font-face."
        ):
        ...
        if view.match_selector(
            start,
            "meta.at-rule.font-palette-values.block.css -meta.descriptor.font-palette-values.",
        ):
            return descriptors.font_palette_values, sublime.INHIBIT_WORD_COMPLETIONS

        # @VIEWPORT DESCRIPTOR NAMES
        if view.match_selector(
            start, "meta.at-rule.viewport.block.css -meta.descriptor.viewport."
        ):
        ...
        if view.match_selector(
            start,
            "meta.at-rule.counter-style.block.css -meta.descriptor.counter-style.",
        ):
        ...
        if view.match_selector(
            start,
            "meta.at-rule.color-profile.block.css -meta.descriptor.color-profile.",
        ):
        ...
        if view.match_selector(
            start, "meta.at-rule.media.css -meta.at-rule.media.block.css"
        ):
        ...
        if view.match_selector(
            start, "meta.at-rule.supports.css -meta.at-rule.supports.block.css"
        ):
        ...
        if view.match_selector(
            start, "meta.at-rule.keyframes.block.css -meta.declaration-list.css"
        ):
...
def handle_at_rule_completions(view, location):
    ...
    if view.match_selector(
        location,
        "meta.at-rule.font-feature-values.block.css -meta.font-feature-type-block.css",
    ):
...
def handle_selector_completions(view, location):
    if view.match_selector(
        location - 1, "punctuation.definition.entity.pseudo-element.css"
    ):
        return selectors.pseudo_elements, sublime.INHIBIT_WORD_COMPLETIONS

    if view.match_selector(
        location - 1, "punctuation.definition.entity.pseudo-class.css"
    ):
        return selectors.pseudo_classes, sublime.INHIBIT_WORD_COMPLETIONS

    if view.match_selector(
        location, "meta.at-rule.keyframes.block.css -meta.declaration-list.css"
    ):
        return selectors.keyframes, sublime.INHIBIT_WORD_COMPLETIONS
